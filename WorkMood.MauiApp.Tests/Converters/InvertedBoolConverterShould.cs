...
/// <summary>
/// Tests for InvertedBoolConverter - MAUI value converter that inverts boolean values
/// Component 12 of Master Test Execution Plan
/// Functionally identical to InverseBoolConverter - testing patterns reused and adapted
/// </summary>
public class InvertedBoolConverterShould
{
    ...
    {
        // Arrange
        var converter1 = new InvertedBoolConverter();
        var converter2 = new InvertedBoolConverter();
        var testValue = true;
    ...
    [Fact]
    public void Should_Have_Identical_Behavior_To_InverseBoolConverter()
    {
        // Arrange
        var invertedConverter = new InvertedBoolConverter();
        var inverseConverter = new InverseBoolConverter();
        var testValues = new object?[] { true, false, null, "test", 42, new object() };

        // Act & Assert - Both converters should behave identically
        foreach (var value in testValues)
        {
            var invertedResult = invertedConverter.Convert(value, typeof(bool), null, CultureInfo.InvariantCulture);
            var inverseResult = inverseConverter.Convert(value, typeof(bool), null, CultureInfo.InvariantCulture);
            
            var invertedBackResult = invertedConverter.ConvertBack(value, typeof(bool), null, CultureInfo.InvariantCulture);
            var inverseBackResult = inverseConverter.ConvertBack(value, typeof(bool), null, CultureInfo.InvariantCulture);

            Assert.Equal(invertedResult, inverseResult);
            Assert.Equal(invertedBackResult, inverseBackResult);
        }
    }
