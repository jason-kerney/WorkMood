...
public interface ILineGraphService
{
    ...
    Task<byte[]> GenerateLineGraphAsync(IEnumerable<MoodEntry> moodEntries, DateRange dateRange, bool showDataPoints, int width = 800, int height = 600);
    ...
    Task SaveLineGraphAsync(IEnumerable<MoodEntry> moodEntries, DateRange dateRange, bool showDataPoints, string filePath, int width = 800, int height = 600);
}