...
public interface ILineGraphService
{
    ...
    Task<byte[]> GenerateLineGraphAsync(IEnumerable<MoodEntry> moodEntries, DateRange dateRange, int width = 800, int height = 600);
    ...
    Task SaveLineGraphAsync(IEnumerable<MoodEntry> moodEntries, DateRange dateRange, string filePath, int width = 800, int height = 600);
}